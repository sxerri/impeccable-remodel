...
0. **Viewability.** If the source asset URLs are root-relative (`href="/..."`, `src="/..."`), the proposed file MUST carry `<base href="{source-origin}">` as the first child of `<head>` before any other work. Without it the source style layer this path exists to preserve is unreachable over `file://`, the page renders as naked DOM plus overlay, and every downstream gate reads garbage. Additive only; note in the run log that viewing requires network and same-page anchors rebase to the origin. The tag is review scaffolding only: strip it before the file leaves the review loop (in stardust pipelines: at handoff to diff/migrate/deploy), so downstream URLs never re-root to the legacy origin.
...
- **Style-layer probe (overlay runs, before the same-site test):** computed checks against overlay-owned properties are circular — they pass whether or not the source layer loaded. Probe at least one SOURCE-layer-owned property (a source framework grid/flex rule, a source container max-width) AND assert zero failed requests for source-path assets (clientlibs, CMS media). If the source layer did not load, every subsequent visual read is invalid; fix viewability first.
